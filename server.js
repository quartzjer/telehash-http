...
{
  var args = mesh.args;
  var io = require("socket.io").listen(args.httpport||0, {log:false});
  ...
  if(!args.http)
  {
    ...
    for (var dev in ifaces) {
      ifaces[dev].forEach(function(details){
        ...
        if(!best || !mesh.isLocalIP(details.address) || best == "127.0.0.1") best = details.address;
      });
  ...
  mesh.pathSet({type:"http","http":args.http});    
